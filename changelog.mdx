...
title: "版本记录"
sidebarTitle: "版本记录"
description: "Huoban v1alpha1 的未发布 Requirement、Preflight、Lifecycle、runner reference 变更与 0.1.0-alpha.0 基线。"
icon:
  name: "history"
  library: "lucide"
---
...
- 可复用 Run materializer、CLI carrier helper 与 golden vectors。
- 用 PRD review 和 incident summary 两个 workflow 验证的实验性线性 planner。
- 验证 `run-materialization` 与 `linear-scheduling` 具名 feature 的机器可读 runtime conformance suite。
- 面向 runner 作者的实现指南，说明 role binding、私有 execution state 与当前执行边界。
...
- reference planner 只推导下一 action；runtime conformance suite 不执行 provider，也不代表完整 runtime 兼容。
