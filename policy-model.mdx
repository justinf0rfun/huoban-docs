...
| 问题 | 负责结构 |
| --- | --- |
| Skill / Adapter 能否解析、程序是否存在、MCP tool 是否可发现？ | `Requirement` + `Preflight` |
| `networkAccess`、`sendMessage`、`publishRemote` 是否允许？ | `Policy` |
| 需要审批时，这一次允许哪些决策动作？ | `Checkpoint` |

当前 reference Preflight 只解析 Policy 引用，不执行 `Policy.spec.rules`。`PreflightReady=True` 不是授权；实现不能因为依赖已就绪而跳过 Policy、Trust 或 Checkpoint。
