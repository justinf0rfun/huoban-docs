...
| print dry-run | Flow、refs、bindings 与分项 hashes 是否已锁入 Run？ | dependency checks。 |
...
Preflight report 的 `snapshotHash` 哈希整个 `Run.spec`。Preflight 还会重算并比对 `flowSpecHash`、`profileRefHash`、`policyRefHash` 和 `bindingHash`；任一缺失或不匹配都会阻断权威结果。修改 Run spec 后必须重新比较或生成 evidence。

这些 hash 锁定声明与选择，不归档每个 Skill 或 Adapter 的完整内容。报告未签名；通过 schema 也不能证明来源、未篡改或时钟可信。
