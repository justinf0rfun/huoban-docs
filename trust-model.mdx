...
| 结构 | 问题 |
| --- | --- |
| `Requirement` | 使用对象前依赖哪些文件、程序、MCP、凭据或上下文来源？ |
| `Preflight` | 这些依赖和引用在当前 Run/workspace 中是否可解析、可用？ |
| `Trust` | 对象来自哪里、是否审查、需要什么 sandbox 与 Policy？ |
...
同样，`PreflightReady=True` 不能替代 Policy、Trust evaluator 或 Checkpoint。实现必须把机械就绪与治理授权分开。
