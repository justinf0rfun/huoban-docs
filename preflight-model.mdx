...
Preflight 对调用输入分四层检查：

1. **契约**：required 输入已绑定；没有未知 binding；binding 类型与 Flow 声明一致。
2. **来源**：stage 的 `flowInput` 已声明；`stageOutput` 存在且来自更早 stage。
3. **约束**：`content` 的 media type 必须在 Flow `mediaTypes` 内；`objectRef` 的 kind 必须在 Flow `objectKinds` 内。
4. **内容**：file carrier 按 raw bytes 复核 hash；`objectRef` 按 kind/name/generation 解析并复核目标对象 canonical `spec` hash。

URL 或 Git blob 不会因为 locator 结构合法就变成 ready。reference CLI 不取回这类内容，其 resolution 可以是 `True`，readiness 必须保持 `Unknown`，因此不能通过 execute gate。若 `objectRef` 指向 Artifact，Artifact 自己的 content hash 仍按独立规则验证。
