...
<CardGroup cols={2}>
  ...
  <Card title="保存预检证据" icon="list-check" type="tip" color="#16A34A">
...
`contentRef` 是共享 locator：Run 的 `content` input binding 也可以直接使用 file、URL 或 Git blob。共享结构不改变对象边界；普通调用输入不是 Artifact，只有产物或协议证据才进入 Artifact 生命周期。
...
| `file` | 仓库或工作区中的文件路径。 |
| `url` | 外部 URL。 |
| `gitBlob` | 由 `repository` 与不可变 `objectId` 定位的 Git blob；reference CLI 不负责取回。 |
| `object` | 内联结构化对象，内容必须放在 `data`。 |
...
| `huoban.dev/preflight-report` | Preflight 的标准结构化证据；该 type 具有额外 schema 约束。 |
...
这个 type 强制要求 `runRef`、`contentRef.type: object` 和符合 Preflight Report fragment 的 `data`。报告保存逐项 Resolution/Readiness 证据；`Run.status.preflight.reportRef` 指向它。

报告不是授权凭证，也不应包含 secret、带凭据 URL、完整 prompt 或 MCP 响应。详见 [Preflight](/preflight-model) 与 [Preflight 规范](/spec-preflight)。
